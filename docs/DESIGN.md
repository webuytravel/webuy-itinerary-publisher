...
抽样 33 个候选,许可证分布是 CC BY-SA 3.0/4.0、CC BY 2.0/3.0 为主,**CC0 只有 2 个**。
也就是说**九成以上要署名**,而 Skybear 产品页没有任何字段放署名(3.3 那张的老问题,
只是这次是规模化的)。三条路,要产品定:

1. **只取 CC0 / 公有领域** —— 按 `LicenseShortName` 过滤即可,代价是候选池砍掉九成;
2. **找一个能放署名的位置** —— Important Note 或 section description 结尾,不好看但合规;
3. **产品页加一个图片版权字段** —— 最干净,要后端排期。

**在这件事定下来之前,Commons 这一级不要开。** 它不是技术问题。
