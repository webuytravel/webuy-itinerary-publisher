...
##### 实测(2026-08-14,生产环境):库是真的,但覆盖面和我们要的不是一回事

拿到 key 当场打了一遍。**接口通、鉴权通、治理字段都是真的**,但对当前这五个产品几乎没用:

| 量到的 | 数字 |
|---|---|
| 图片素材可检索总数 | **5,956** |
| 其中 `content_category=product_experience`(景点/产品图) | **1,013** |
| 再过版权(`owned`/`licensed`)+ 净度(`clean`)+ 非广告 + 无烫字 | **395** |
| 其余 | 约 96% 是 `customer_moment` / `customer_proof`,即客户合影 |

**395 张的地理分布是中国主流线路**:北京 90、张家界 62、上海 54、四川 44、西藏 42、
成都 31、西安 30、哈尔滨 30、重庆 27、深圳 27、故宫 26、九寨沟 26、长城 22……
另有大量土耳其(Istanbul 59、Cappadocia 55)和越南,因为这个库主要服务
`wetrip`(US)和 `webuytravel_id`(ID)两个品牌。

**而我们这五个产品全在长尾**:宁夏、山西内蒙、北疆、贵州、粤。34 个主体逐个查,
过全部安全闸的只有 9 张(云冈石窟 2、大同 1、乌鲁木齐 1、洪崖洞 2、广州 3)。
按 token 匹配跑完 126 个景点条目:

| 产品 | 条目 | 现有图 | Commons | **中台 A0** | 三者并集 | 仍缺 |
|---|---:|---:|---:|---:|---:|---:|
| WBCKWE | 21 | 13 | 13 | 1 | 19 | 2 |
| WBCURC | 35 | 14 | 16 | 0 | 21 | 14 |
| WBCHET | 31 | 17 | 12 | 1 | 22 | 9 |
| WBINC9 | 19 | 16 | 3 | 0 | 17 | 2 |
| WBSZX1 | 20 | 11 | 6 | 2 | 15 | 5 |
| **合计** | **126** | 71 | 50 | **4** | **94(74%)** | **32** |

**A0 只把并集从 93 推到 94。**

**结论要分两句说,不要合并:**

1. **对眼下这批产品,A0 解决不了问题**——它覆盖的是主流线路,我们做的是长尾。
   不值得为这五个产品去接。
2. **对以后的产品,A0 是最该接的一条**——北京/张家界/四川/西藏/西安/哈尔滨/九寨沟
   这些线路它盖得很厚,而且**它是所有图源里唯一一个版权、肖像、净度都已经治理成
   字段的**,不用人去看图判断能不能用。真正该做的是**反过来把我们选定的图回流进
   这个库**,让长尾也长出来。

顺带一条:那 96% 的客户合影不是废料,只是**不属于这个用途**——产品页要的是景点,
不是客人。文档也写明只有 `customer_provided` 目录的 `customer_moment` 才有
`trust_building` 场景,而且**所有视频在转码和 16MB 治理完成前一律不可自动对客发送**。
...
### 3.6.1 剩下 30 张真照片:9 张没进去,原因不是尺寸

> ⚠️ **这一节的第一版结论是错的,原样留着改正过程。**
> 第一版写的是「7 张真照片只因为裁后 420–610px 卡在 `MIN_PDF_CROP_WIDTH=632` 上」。
> 核对时发现**那 7 张里有 3 张本来就在 `plan.json` 里**(WBCKWE p4#0 513px、
> WBCURC p4#0 492px、WBINC9 p2#2 590px)。既然 513px 的能进,632 这条线显然
> 没在拦它们。**下面是重新查的。**

`MIN_PDF_CROP_WIDTH` **在生产路径上根本没有生效**。它只被 `PdfImageSet.usable`
用到,而 `usable` 只被 `lib/image_plan.build()` 用到,**而 `build()` 从来没被调用过**
——和 3.2 那个 `DaySection.search_subjects` 是同一段死代码。`bin/compose.py` 自己
做匹配,**全程不看尺寸**。

**这是同一个模式的第三次出现**(前两次:6.11 的 `DaySection.search_subjects`、
3.2 的 `photo_source` 整条链)。仓库里有两套实现,写文档时读的是没跑的那套。
**以后要判断「某条规则有没有在起作用」,先 grep 调用方,不要读实现。**

#### 30 张真照片的实际去向

| | 张数 |
|---|---:|
| `kind=photo` 且没被闸门拒的栅格 | **30** |
| 其中有人看过并标了主体(`verdict` = real/asset) | 27 |
| **被人判为不可用**(`generic` 1 + `cgi_suspect` 2) | **3** |
| 最终进了 `plan.json` | **21** |

**WBCHET 一个产品占掉了那 3 张全部**,这就是它册子图为零的原因——不是尺寸,
是**人看过之后否掉的**:

- `p3#0` 草地上一棵孤树 → `generic`(哪里都可能,不能指认内蒙古)
- `p3#2` 响沙湾莲花酒店 → `cgi_suspect`(效果图,不是实拍)
- `p4#3` 万年冰洞 → `cgi_suspect`

只剩 `p4#0` 悬空寺是 `real`,而它也没进 plan——因为第 6 天被 `OVERRIDES` 显式
指定了云冈石窟和大同古城两张,把名额占满了。
...
| 产品 | 尺寸 | 主体 | 没进去的原因 |
|---|---|---|---|
| WBCKWE | 721×335 | 民族歌舞表演 | token 匹配没配上任何一天 |
| WBCKWE | 821×458 | 西江千户苗寨 | 同上 —— **这是那条线路的头牌景点** |
| WBCURC | 763×315 | 歌舞演出 | 同上 |
| WBCHET | 719×499 | 悬空寺 | 第 6 天名额被 `OVERRIDES` 占满 |
| WBSZX1 | 991×369 | 广东千古情舞台 | 6.7 记过:裁到 section 要放大 2.44×,被挤掉 |
| WBSZX1 | 727×803 | 珠海日月贝大剧院 | token 匹配没配上 |

**六张里有五张是「配额」问题,不是「质量」问题**:一天只放两张 section 图,
`OVERRIDES` 又优先,册子自己的照片就排不上。**景点卡(Trip Photos)这一层
正好是它们的位置**——每天的景点卡有好几张,不跟 section 抢名额。

所以 TRIP 槽的价值不在「放宽尺寸救回照片」(那个说法是错的),而在
**多出一层槽位,让册子照片不必和 `OVERRIDES` 抢那两个名额**。

### 3.6.2 Trip Photos 该按多大编码:量出来的

阈值 `MIN_PDF_CROP_WIDTH = 632` 虽然没生效,但它背后的问题是真的:
`SECTION` 目标 1200px 从哪来的?**没有量过。**
...
**所以 TRIP 槽的目标定在 1080×810、上限 2.2**:1080 长边正是自家图库在售图的
标准(见 `catalogue_source.py` 开头),不是新立的杆;2.2 比 SECTION 的 1.90 松,
因为一张 492px 的册子裁图在 535px 的灯箱里只是 1.09×。

**没有去动 `MIN_PDF_CROP_WIDTH`。** 它现在是死的(见 3.6.1),但如果哪天
`build()` 被接回主流程,SECTION 仍然应该守住 632——它是唯一在页面上以大图
出现过的日程图,降它的 floor 是拿一个看得见的槽换一个看不见的。
...
### 6.05 Trip Photos 这一层上线了(2026-08-14)

126 个景点卡从**全部为空**变成 88 个有图,五个产品合计 154 张 trip 图。
逐个回读核对过,五个仍然是 `Unpublished`。

| 产品 | id | section | 覆盖天 | trip 图 | 卡覆盖 |
|---|---|---:|---|---:|---|
| WBCKWE | 408 | 10 | 8/9 | 29 | 16/21 |
| WBCURC | 409 | 14 | 12/13 | 39 | 25/35 |
| WBCHET | 410 | 10 | 7/9 | 33 | 19/31 |
| WBINC9 | 411 | 11 | 7/9 | 30 | 16/19 |
| WBSZX1 | 412 | 12 | 7/7 | 23 | 12/20 |

顺带补上的 section:409 的 D1 和 D12(6.11 里静默留空的两天)、411 的 D2
(唯一一条「有景点却四级图源全废」的 gap)、410 的 D7 换成乌兰哈达火山。

#### 第一次传上去有 64 张和 section 重了(当天修掉)

业务同事看完生产页面提的:**section 的图不能和它下面景点卡的图相同,景点卡之间
也不能重复。** 逐条量了一遍,两条都破得很厉害:
...
成因就在 `assign_trip_photos` 自己:池子里含 section placement,而 section 图按定义
就是那天的照片,`score` 给它们的分最高——于是每张卡都优先拿到自己头顶那张。
`max_reuse=2` 又允许同一张用两次。

修法是两条硬规则:**当天 section 用掉的源文件,那天的卡一律不许用**;
`max_reuse` 默认降到 **1**,即一个文件在整个 trip 层只出现一次。
...
**跨天复用只多 4 张卡,不值得为它保留可见的重复。**

#### Trip Photos 的槽位怎么定位:ref = SEC + 29×(卡序+1)

`file_upload` 要 `read_page`/`find` 给的 `ref`,而一个 13 天产品有 40+ 个文件
input。实测这些 ref 在同一个 section 内是**等差的**:

```
Section Photos = S,  第 k 张卡(0 基) = S + 29×(k+1)
```

五个产品、五十多个 section 全部符合。**跨 section 的步长不固定**(见到过 +28
和 +31 两种),所以**只推导 section 内部,section 本身的 ref 要逐个 find**。

> ⚠️ **`find` 返回的 "Trip Item N" 标注会错位。** WBCURC 第 6 天只有 3 张卡,
> 一次 `find` 却把三个 ref 标成了 Trip Item 2/3/4。**数量对不上时不要顺着它猜**
> ——换一句只问那一个 section 的 query 重查,或者用上面的等差关系验算。
> 传错槽的代价是一张图挂到别的景点卡上,而且看列表数字看不出来。

#### 两个操作细节

- **先展开全部 section 再取 ref。** 折叠状态下里面的 file input 拿不到。
  展开是纯前端状态,不写库(见 UPLOAD_RUNBOOK 6.2)。
- **截图偶尔全白,是渲染时序不是页面出错。** 上传后立刻 `scrollIntoView` +
  截图,五次里有三次拍到空白帧。补一次 `scrollTo(0, scrollHeight)` → 等 1.5s →
  再 `scrollIntoView` → 再等 1.5s 就正常了。**组件模型的回读值始终是对的**,
  以它为准,截图只是给人看的二次确认。

### 6.06 两个问题:还有重复,以及「色彩灰暗」(2026-08-14 业务同事看生产页面)

#### 一、重复没清干净:按路径去重漏了同图异名

第一轮修完之后仍有一处重复:**WBCHET 第 6 天的卡 2 和卡 3 是同一张云冈石窟**。
按 `src_path` 去重看不出来,按内容 sha1 一眼就出来——Commons 的
`d06_yungang_grottoes_datong` 和 `d06_yungang_grottoes_buddha_statues` 两个块
**下载到了同一张照片,存成了两个文件名**。

`assign_trip_photos` 的去重键因此改成**文件内容的 sha1**,不是路径。
五个产品重新核对:与 section 同图 0、日内重复 0、全局重复 0。

#### 二、色彩灰暗:我们只有准确性这一根轴

这条是更根本的。业务同事的原话是「色彩不要灰暗,要明亮的、适合对外展示,
可以参考 webuy Itinerary Create 那边的选图标准」。去读了那边的代码
(`webuy-itinerary-creation/scripts/fetch_landscape_photos.py`),**它是两轴的**:

| | 那边(印刷册子) | 这边(上线之前) |
|---|---|---|
| 准确性 | `_verify_match`(Gemini 看图) | GPS + 人看接触印相 |
| **美学** | **`_aesthetic_score`(Gemini 扮演旅游杂志图片编辑,0–10)** | **没有** |
| 源优先级 | Shutterstock → Unsplash/Pexels → **Commons「documentary last resort」** | **Commons 提成了主力** |

那边给 Gemini 的提示词把标准写死了:

> Score **LOW for dull, grey, cluttered, flat, or snapshot-like** images;
> score **HIGH for striking, vivid, well-composed, postcard-worthy** shots.

**我们做的正好相反。** 为了地理可验证(Commons 带 GPS),把那边明确排在最后的
「档案馆图源」提成了主力,而档案馆照片天然就是平实、非营销的。**准确性买到了,
代价是美学**,而且因为没有这根轴,连代价有多大都不知道。

#### 量出来:问题不是暗,是灰

拿 48 张自家在售图库图当房子标准(它们就是 `webuytravel.sg` 上正在卖的图):

| | 亮度 | **饱和度** | 对比 |
|---|---|---|---|
| **自家图库(房子标准)** | 0.482 | **0.421** | 0.217 |
| WBCKWE trip 图 | 0.440 | **0.235**(房子的 56%) | 0.239 |
| WBINC9 | 0.488 | **0.272** | 0.196 |
| WBSZX1 | 0.436 | **0.303** | 0.189 |
| WBCHET | 0.507 | **0.312** | 0.188 |
| WBCURC | 0.491 | **0.319** | 0.206 |

**亮度全部正常**(0.44–0.51 对房子的 0.482),**五个产品的饱和度无一达标**。
所以「灰暗」这个词里,真正成立的是「灰」不是「暗」——数字和肉眼是一致的。

按房子第 10 百分位设闸门(`lib/image_appeal.py`),**112 张里 46 张不达标**。
最差的几张恰恰是我为「准确」挑的:梵净山雾中拱桥饱和 0.037–0.061(几乎是黑白)、
深中通道 0.035(灰海灰天)、悬空寺 0.112、乌兰哈达火山 0.140。

#### 好消息:不用重新取图

把每个块内的候选按美学分重排,**118 条选片里有 67 条在同一个块内就有明显更亮的
替代**(>15%)——同一个块意味着准确性已经过关,只是当初挑的时候没有这根轴。

**但美学分不能单独作数。** WBCURC 火焰山那一组,分最高的是山下的骆驼(1.09),
而山体本身是 0.92——只看分数会把那天的主角换成配角。所以规则仍是两轴:
**美学分做强先验,主体价值由人定**,和那边 `_verify_match` + `_aesthetic_score`
的分工是同一套。
