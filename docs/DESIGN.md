...
**代码这边该改的是**:`file` 分支不能再硬编码 credit/license,应当由那一条
选片(现在是 `editorial.json` 的 `section_overrides`)显式提供,提供不了就拒绝
——这正是 6.9 那条判断标准的又一次应用。**这一条到 2026-09-11 仍未做。**
...
#### 上线结果(2026-08-14 第三轮,最终)

| 产品 | id | trip | 卡覆盖 | 平均饱和 | 仍不达标 |
|---|---|---:|---|---|---:|
| WBCKWE | 408 | 18 | 12/21 | 0.310 | 5 |
| WBCURC | 409 | 27 | 17/35 | 0.367 | 5 |
| WBCHET | 410 | 20 | 13/31 | 0.338 | 3 |
| WBINC9 | 411 | 17 | 10/19 | 0.275 | 9 |
| WBSZX1 | 412 | 19 | 10/20 | 0.386 | 9 |
| **合计** | | **101** | **62/126** | | **31** |

三次收紧的完整代价:**88 卡(有重复)→ 66 卡(去重复)→ 62 卡(去灰暗)**。
每一次都是拿覆盖换质量,每一次都是业务同事看生产页面提出来的。

**剩下 64 张空卡不是可以再挑一轮解决的。** 现有四级图源在这五条线路上已经用尽,
WBINC9 更是连重挑都没有改善空间(0.272 → 0.275,题材本身是米色的)。
要同时把覆盖和色彩往上推,唯一的路是补图源——地接社实拍或中文商业图库(3.5 的 C1/C3)。
...
成品 6 条连同取舍理由写在 `work/<CODE>/editorial.json` 的 `highlights` 里
(2026-09-11 之前在 `bin/make_payload.py` 的 `HOUSE_HIGHLIGHTS`,见 issue #4)。
...
### 6.9.1 又一处:路线图槽位填错了也没人看(2026-09-11 ACKMG12T)

ACKMG12T 第 7 页把整页「Special Terms and Conditions」栅格化成一张 978×854 的图:
淡蓝底,深蓝和橙色正文,**整张图里一张照片都没有**。`_looks_like_route_map` 把它判成
`route_map`,而 `bin/compose.py` 取第一张 `route_map` 填 `wt_travel.routeMapUrl`——
**一页退改条款会作为产品的路线图上线,流程全程不报错。**

分类器没写错,是问题问错了。它量的三项——饱和度 0.09、最大色块占比 0.55、色桶 13——
回答的是「这不是照片」,不是「这是示意图」。一页铺满正文的版式同样满足这三项,
所以**「不是照片」和「是示意图」必须分开问**。

现在在 `extract` 里先过一道**文字页闸门**,量的是暗像素的行投影:一段正文是一把梳子,
每行字咬出一条有墨的行带,每个行距留下一条空行,间距一路几乎不变;示意图的墨在
海岸线、大巴路线和城市图钉落的地方,没有这个节奏。两项一起要求——行投影高通之后的
自相关峰 ≥ 0.45,且空行占比 ≥ 30%。十本册子 75 张过闸栅格实测:

| | 自相关峰 | 空行占比 |
|---|---|---|
| 两页条款(ACKMG12T p7 / p8) | 0.63 / 0.82 | 43–48% |
| 八张路线图 | 0.15–0.35 | 12–21% |
| 全部照片 | ≤ 0.37 | — |

高通这一步不能省:不减掉整幅的上下明暗趋势,每张图都在自己的渐变上拿 0.6–0.99,
三类完全分不开。第二项是为**最接近的那张照片**加的——一张湖边寺庙倒影,对称结构
让它拿到 0.37 的梳齿,但只有 6% 空行。

文字页判成 `furniture` 并记下拒绝理由,**不是**留给路线图或照片槽:978×854 过得了
所有几何闸门,裁成 4:3 还是 hero 级,只教会路线图分类器只会把它从一个错槽挪到
另一个错槽。

**但这条的根子还是 6.9:** 路线图槽位不进每日 GAP 统计,审核页在该槽为空时直接
`continue`——**填错了没人看,填不上也没人看。** 这次是人打开那张图才发现的,
不是流程发现的。按 6.9 的判据,这一槽也该响,还没有做。
...
### 6.13 编辑决策搬出源码(2026-09-11,issue #4)

ACKMG12T 做完之后量了一次:5 个脚本里 8 张以产品代码为键的字典,约 1352 行;
`bin/compose.py` 全文 1624 行,其中 1119 行(69%)是数据不是逻辑。后果是业务
同事跑一遍册子就要有工程跟在后面提交代码,两个人同时做两本会改到同一张表的
相邻行。全部搬到 `work/<CODE>/editorial.json`(格式见 `docs/EDITORIAL_JSON.md`)。

**这件事唯一靠得住的验收方式是等价性证据**,因为它动的是 17 个**已经签过字**的
产品的配图解析路径:如果不是等价变换,后果是线上产品的图悄悄换掉,而管线里
没有任何一处会报错 —— 摘要行只数张数,审核页只渲染 plan 里有什么。这正是 6.9
那条判断标准的场景,只不过这次连「响」的机会都没有,所以证据要在合入之前拿到。

三条证据,都可重跑:

| 跑什么 | 证明什么 |
|---|---|
| `tools/migrate_editorial.py` | JSON 是从迁移前的源码机械抽出来的,重跑逐字节一致 |
| 把 8 张表从 JSON 反向重建,和迁移前的字面量比 | 数据本身没动 |
| `tools/verify_equivalence.py` | 迁移前后两版 `compose.py` 各跑 17 个产品,plan 逐字节一致 |

**跑不到的那一段要说出来。** `materialise()` 要的原始栅格(`work/**/cand/`、
`cand_commons/`、`cat/`、`raw/`)是 gitignored 的,本机一个产品都不齐
(最好的三个是 24/40、26/38、24/51,其余 14 个是 0),所以 17 个产品**没有一个**
能真的跑完 `bin/compose.py` 落地 `plan.json`。比对因此停在 `assign_trip_photos`
之后、`materialise` 之前 —— 那正好是这次重构改到的全部范围,而 `materialise` /
`render` 两步在 `lib/` 里一行没动。为了不让内容哈希去重那条路完全不被走到,
同一套比对在「有栅格」和「无栅格」两种环境下各跑了一遍,两次都全同。

**换成 JSON 之后最容易退化的是「键不认识,跳过」**,也就是 6.9 / 6.11 那条老病根。
所以 `lib/editorial.py` 反过来:任何不认识的键、不认识的取值、少掉的必填字段,
都在读取那一刻退出,错误信息带产品代码和它在文件里的位置。指向不存在的候选那一类
仍然由 `bin/compose.py` 炸(只有它知道 `candidates.json` 里有什么)。

独立评审(2026-09-11)在这上面找出两个洞,都已补,都值得记下来,因为它们是
**同一种病的两个新变体**:

* **形状对、日期不存在。** `d40`(本意 `d04`)过得了 schema —— 它只管 `dNN` 的
  字面形式;而 `compose()` 只遍历行程里真有的天,所以那条人工选片既不会被解析、
  也走不到「候选不存在」那道检查。第 4 天于是悄悄退回自动匹配,摘要行一切正常。
  这个洞在旧源码里同样存在。现在 `compose()` 开头先比一遍日期键和行程天数。
* **写错类型被当成「没写」。** `doc.get(k) or {}` 会把 `false` / `0` / `""` / `[]`
  折成一个合法的空容器,读起来就像「这个产品没有人工选片」。**缺字段**才等于
  没有编辑决策;写了就必须是对的形状。

判断标准仍是 6.9 那一条,只是这次要多问一句:**不光「读到了错的东西要炸」,
还有「本该读到的东西根本没被读到,也要炸」。**

两种错法在 `tests/test_editorial.py` 里都有测试,而且逐条做过变异验证:把
stock 那一支改回 `continue`、把 `check_days` 删掉、把两个严格类型检查改回 `or`、
把 section 和 trip 两支的 `credit`/`license` 参数删掉 —— 五处各让至少一条测试变红。
