...
2. **`bin/compose.py`,解析引用时。** 指向一个不存在的候选(block 名打错、
   ...
   **日期键也在这里查。** `d40`(本意是 `d04`)形状是对的,schema 挑不出来;
   而 compose 只遍历行程里真有的天,所以那条选片既进不了 plan、也走不到上面
   那道候选检查 —— 不专门查一遍的话,第 4 天会悄悄退回自动匹配,摘要行一切正常:

   ```
   WBTEST section_overrides.d40: 行程里没有这一天 —— 多余的日期键 ['d40'],
     这本行程只有 ['d01', 'd02', 'd03']
   ```

以上都有测试守着(`tests/test_editorial.py`),而且逐条验过「把那一行删掉会红」。

一个配套的性质:**写错类型不等于没写。** `"section_overrides": false` / `0` /
`""` / `[]` 全部当场退出,而不是被读成一个空对象 —— 后者读起来就像「这个产品
没有人工选片」,于是静默走回自动匹配。**缺字段**才等于没有编辑决策。
